...
function TableGen({name} : TableInterface){
	...
	}, [setHeader, setRows, name])

	let i = 0
	let final_rows = rows.map( (list,index) => { i=i+1; let j=0; return <tr>{list.map((element,index) => {let key = name+`[${i}] ${header[j]}`; j+=1; return <td id={key} key={key}>{element}</td>})}</tr>})
